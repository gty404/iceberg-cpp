...
TEST_F(ManifestFilterManagerTest, DropDeleteFilesOlderThan_KeepsNewerEntries) {
  auto* metadata = table_->metadata().get();
  auto factory = MakeWriterFactory(*metadata);
...
TEST_F(ManifestFilterManagerTest, RemoveDanglingDeletesFor_FiltersDanglingDV) {
  auto* metadata = table_->metadata().get();
  auto factory = MakeWriterFactory(*metadata);
