...
TEST_F(MergingSnapshotUpdateTest, AddsDataFiles_InitiallyFalse) {
...
TEST_F(MergingSnapshotUpdateTest, AddsDataFiles_TrueAfterAdd) {
  ICEBERG_UNWRAP_OR_FAIL(auto op, NewMergeAppend());
...
TEST_F(MergingSnapshotUpdateTest, AddsDeleteFiles_TrueAfterAdd) {
...
TEST_F(MergingSnapshotUpdateTest, DeletesDataFiles_TrueAfterRegisterDelete) {
  CommitFileA();
...
TEST_F(MergingSnapshotUpdateTest, CleanUncommitted_ClearsCaches) {
  ICEBERG_UNWRAP_OR_FAIL(auto op, NewMergeAppend());
  EXPECT_THAT(op->AddFile(file_a_), IsOk());
  EXPECT_THAT(op->Commit(), IsOk());

  // CleanUncommitted with an empty set should delete everything and not crash.
  op->CleanUncommitted({});
}
...
TEST_F(MergingSnapshotUpdateTest, CommitDeleteFile_SummaryHasAddedDeleteFiles) {
  auto del_file = MakeDeleteFile("/delete/del_a.parquet", 1L);
...
// Java: deleteFilterManager.buildSummary(filteredDeletes) handles this.
// C++: loop over delete_filter_manager_.FilesToBeDeleted() after Step 3.
TEST_F(MergingSnapshotUpdateTest, CommitDeletesDeleteFile_SummaryHasRemovedDeleteFiles) {
...
TEST_F(MergingSnapshotUpdateTest, DuplicateDataFile_OnlyCountedOnce) {
  ICEBERG_UNWRAP_OR_FAIL(auto op, NewMergeAppend());
  EXPECT_THAT(op->AddFile(file_a_), IsOk());
...
TEST_F(MergingSnapshotUpdateV1Test, ValidateNewDeleteFile_V1Rejected) {
  auto del_file = MakeDeleteFile("/delete/del_a.parquet");
  ICEBERG_UNWRAP_OR_FAIL(auto op, NewMergeAppend());
  EXPECT_THAT(op->AddDelete(del_file), IsError(ErrorKind::kInvalidArgument));
}

TEST_F(MergingSnapshotUpdateTest, ValidateNewDeleteFile_V2RejectsDeletionVector) {
...
TEST_F(MergingSnapshotUpdateTest, ValidateNewDeleteFile_V2AllowsEqualityDelete) {
...
TEST_F(MergingSnapshotUpdateTest, AddManifest_RejectsDeleteManifest) {
...
TEST_F(MergingSnapshotUpdateTest, AddManifest_RejectsManifestWithExistingFiles) {
...
TEST_F(MergingSnapshotUpdateTest, AddManifest_RejectsManifestWithDeletedFiles) {
  ManifestFile manifest;
...
TEST_F(MergingSnapshotUpdateTest, AddManifest_RejectsManifestWithAssignedSnapshotId) {
  ManifestFile manifest;
...
TEST_F(MergingSnapshotUpdateTest, AddManifest_RejectsManifestWithFirstRowId) {
  ManifestFile manifest;
...
TEST_F(MergingSnapshotUpdateTest, AppendManifest_EmptyTable) {
...
TEST_F(MergingSnapshotUpdateTest, AppendManifest_WithDataFiles) {
...
TEST_F(MergingSnapshotUpdateTest, AppendManifest_MergeWithMinCountOne) {
...
TEST_F(MergingSnapshotUpdateTest, AppendManifest_DoNotMergeMinCount) {
...
TEST_F(MergingSnapshotUpdateTest, ManifestMerge_MergesIntoOne) {
...
TEST_F(MergingSnapshotUpdateTest, ManifestMerge_DoesNotMergeWhenBelowMinCount) {
...
TEST_F(MergingSnapshotUpdateTest, ManifestMerge_DoesNotMergeWhenSizeTargetTooSmall) {
...
TEST_F(MergingSnapshotUpdateTest, Summary_ManifestCountsOnFirstCommit) {
  ICEBERG_UNWRAP_OR_FAIL(auto op, NewMergeAppend());
  EXPECT_THAT(op->AddFile(file_a_), IsOk());
  EXPECT_THAT(op->Commit(), IsOk());
...
TEST_F(MergingSnapshotUpdateTest, Summary_ManifestCountsOnSecondCommitNoMerge) {
  CommitFileA();
...
TEST_F(MergingSnapshotUpdateTest, Summary_ManifestCountsAfterMerge) {
  ICEBERG_UNWRAP_OR_FAIL(auto props, table_->NewUpdateProperties());
  props->Set(std::string(TableProperties::kManifestMinMergeCount.key()), "1");
  EXPECT_THAT(props->Commit(), IsOk());
  EXPECT_THAT(table_->Refresh(), IsOk());
...
TEST_F(MergingSnapshotUpdateTest, Summary_ManifestCountsAfterDelete) {
  ICEBERG_UNWRAP_OR_FAIL(auto props, table_->NewUpdateProperties());
  props->Set(std::string(TableProperties::kManifestMinMergeCount.key()), "1");
  EXPECT_THAT(props->Commit(), IsOk());
  EXPECT_THAT(table_->Refresh(), IsOk());
...
TEST_F(MergingSnapshotUpdateTest, DataSpec_ThrowsWithMultipleSpecs) {
  ICEBERG_UNWRAP_OR_FAIL(auto op, NewMergeAppend());
...
TEST_F(MergingSnapshotUpdateTest, DataSpec_ThrowsWhenEmpty) {
  ICEBERG_UNWRAP_OR_FAIL(auto op, NewMergeAppend());
