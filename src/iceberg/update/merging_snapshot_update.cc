...
Result<std::vector<ManifestFile>> MergingSnapshotUpdate::WriteNewDeleteManifests() {
  ...
  std::unordered_map<int32_t, std::vector<std::shared_ptr<DataFile>>>
      delete_files_by_spec;
  for (const auto& file : new_delete_files_) {
    delete_files_by_spec[file->partition_spec_id.value()].push_back(file);
  }

  std::vector<ManifestFile> result;
  for (const auto& [spec_id, delete_files] : delete_files_by_spec) {
    ICEBERG_ASSIGN_OR_RAISE(auto spec, base().PartitionSpecById(spec_id));
    ICEBERG_ASSIGN_OR_RAISE(auto written,
                            WriteDeleteManifests(std::span(delete_files), spec));
    for (const auto& m : written) {
      all_written_manifests_.insert(m.manifest_path);
    }
    result.insert(result.end(), std::make_move_iterator(written.begin()),
