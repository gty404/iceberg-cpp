...
Status MergingSnapshotUpdate::AddDeleteFile(std::shared_ptr<DataFile> file) {
  ...
  new_delete_files_.push_back(std::move(file));
  return {};
}
...
Result<std::shared_ptr<PartitionSpec>> MergingSnapshotUpdate::DataSpec() const {
  if (new_data_files_by_spec_.size() != 1) {
    return InvalidArgument("DataSpec() requires exactly one partition spec; got {}",
...
Status MergingSnapshotUpdate::AddDeleteFile(std::shared_ptr<DataFile> /*file*/,
  ...
  return NotImplemented(
      "AddDeleteFile with explicit data sequence number is not yet implemented");
}
...
Status MergingSnapshotUpdate::AddManifest(ManifestFile manifest) {
  ...
  if (manifest.has_existing_files()) {
    return InvalidArgument("Cannot append manifest with existing files: {}",
  ...
  if (manifest.added_snapshot_id != kInvalidSnapshotId) {
    return InvalidArgument("Snapshot id must be assigned during commit: {}",
                           manifest.manifest_path);
  }
  if (manifest.first_row_id.has_value()) {
    return InvalidArgument("Cannot append manifest with assigned first_row_id: {}",
                           manifest.manifest_path);
  }
...
ManifestWriterFactory MergingSnapshotUpdate::MakeTrackedWriterFactory() {
  return [this](int32_t spec_id,
    ...
    ICEBERG_ASSIGN_OR_RAISE(auto schema, meta.Schema());
    ...
    return ManifestWriter::MakeWriter(meta.format_version, SnapshotId(), std::move(path),
                                      ctx_->table->io(), std::move(spec),
                                      std::move(schema), content);
  };
}
...
Result<std::vector<ManifestFile>> MergingSnapshotUpdate::WriteNewDeleteManifests() {
  ...
  if (!new_delete_files_.empty()) {
    ICEBERG_ASSIGN_OR_RAISE(auto spec, base().PartitionSpec());
    ICEBERG_ASSIGN_OR_RAISE(auto written,
                            WriteDeleteManifests(std::span(new_delete_files_), spec));
    for (const auto& m : written) {
      all_written_manifests_.insert(m.manifest_path);
    }
    result.insert(result.end(), std::make_move_iterator(written.begin()),
...
Result<std::vector<ManifestFile>> MergingSnapshotUpdate::Apply(
  ...
  for (const auto& file : new_delete_files_) {
    ICEBERG_RETURN_UNEXPECTED(ValidateNewDeleteFile(*file));
  }
  ...
  auto tracked_factory = MakeTrackedWriterFactory();
  ...
  // those are rewritten manifests whose sequence number will be assigned at commit time.
  ...
  for (auto m : append_manifests_) {
    m.added_snapshot_id = snapshot_id;
    new_data_manifests.push_back(std::move(m));
  }
  for (auto m : rewritten_append_manifests_) {
    m.added_snapshot_id = snapshot_id;
    new_data_manifests.push_back(std::move(m));
  }
...
Status MergingSnapshotUpdate::ValidateAddedDataFiles(
  ...
  ICEBERG_ASSIGN_OR_RAISE(auto ancestors,
                          SnapshotUtil::AncestorsBetween(metadata, parent->snapshot_id,
                                                         starting_snapshot_id));

  // Only scan manifests from APPEND and OVERWRITE snapshots — those are the only
  // operations that add data files.
  std::unordered_set<int64_t> matching_snapshot_ids;
  for (const auto& snap : ancestors) {
    auto op = snap->Operation();
    if (op == DataOperation::kAppend || op == DataOperation::kOverwrite) {
...
Status MergingSnapshotUpdate::ValidateDataFilesExist(
  ...
  // Build the set of snapshot IDs to scan: OVERWRITE and REPLACE always included;
  // DELETE included only when allow_deletes is false.
  std::unordered_set<int64_t> matching_snapshot_ids;
  for (const auto& snap : ancestors) {
    auto op = snap->Operation();
    if (!op.has_value()) {
...
Status MergingSnapshotUpdate::ValidateAddedDataFiles(
  ...
  return NotImplemented(
      "ValidateAddedDataFiles with PartitionSet is not yet implemented");
}
...
Status MergingSnapshotUpdate::ValidateNoNewDeletesForDataFiles(
  ...
  return NotImplemented(
      "ValidateNoNewDeletesForDataFiles with data filter is not yet implemented");
}
...
Status MergingSnapshotUpdate::ValidateNoNewDeleteFiles(
    ...
    const std::shared_ptr<Snapshot>& /*parent*/, std::shared_ptr<FileIO> /*io*/) {
  return NotImplemented(
      "ValidateNoNewDeleteFiles with Expression is not yet implemented");
}
...
Status MergingSnapshotUpdate::ValidateNoNewDeleteFiles(
    ...
    std::shared_ptr<FileIO> /*io*/) {
  return NotImplemented(
      "ValidateNoNewDeleteFiles with PartitionSet is not yet implemented");
}
...
Status MergingSnapshotUpdate::ValidateDeletedDataFiles(
    ...
    const std::shared_ptr<Snapshot>& /*parent*/, std::shared_ptr<FileIO> /*io*/) {
  return NotImplemented(
      "ValidateDeletedDataFiles with Expression is not yet implemented");
}
...
Status MergingSnapshotUpdate::ValidateDeletedDataFiles(
    ...
    std::shared_ptr<FileIO> /*io*/) {
  return NotImplemented(
      "ValidateDeletedDataFiles with PartitionSet is not yet implemented");
}
...
Result<std::unique_ptr<DeleteFileIndex>> MergingSnapshotUpdate::AddedDeleteFiles(
  ...
  // Collect delete manifests from OVERWRITE and DELETE snapshots only.
  std::unordered_set<int64_t> matching_snapshot_ids;
  for (const auto& snap : ancestors) {
    auto op = snap->Operation();
    if (op == DataOperation::kOverwrite || op == DataOperation::kDelete) {
      matching_snapshot_ids.insert(snap->snapshot_id);
    }
  }
  ...
  for (const auto& snapshot : ancestors) {
    ...
    auto cached = SnapshotCache(snapshot.get());
    ICEBERG_ASSIGN_OR_RAISE(auto manifests, cached.DeleteManifests(io));
    for (const auto& m : manifests) {
      if (matching_snapshot_ids.contains(m.added_snapshot_id)) {
        delete_manifests.push_back(m);
      }
    }
  }
